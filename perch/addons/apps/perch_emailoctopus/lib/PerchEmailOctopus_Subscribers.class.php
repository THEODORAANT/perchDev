...
class PerchEmailOctopus_Subscribers extends PerchEmailOctopus_Factory
{
...
$opts=array();
$opts["url"]="/lists/c8e227b4-b7bb-11ef-b6ef-0de94d1f09a3/contacts";
$opts["data"]=$data;
$result = $Factory->curl_api($opts);
	...
	public function import_next(Perchperch_emailoctopus_Import $Import)
	{
		$Lists = new PerchEmailOctopus_Lists($this->api);
		...
		$perch_emailoctopusAPI = $this->get_api_instance();
		$listID       = $List->listperch_emailoctopusID();

		$members = $Mperch_emailoctopusAPI->get("lists/$listID/members", [
			...
			if (isset($members['members']) && PerchUtil::count($members['members'])) {
				...
				foreach($all_members as $member) {
					...
					if (!$this->remote_subscriber_exists_locally($member['unique_email_id'])) {
						...
						// subscriber exists
						$Subscriber = $this->get_one_by('subscriberperch_emailoctopusID', $member['unique_email_id']);
	...
	public function lookup_and_create(array $data)
	{
		...
		//$hash 		  = ->subscriberHash($data['email']);

		//$member = ->get("lists/$listID/members/$hash");
	...
	{
		return [
				'subscriberemailoctopusID' => $member['unique_email_id'],
