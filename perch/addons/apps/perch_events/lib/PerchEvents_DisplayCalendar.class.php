...
class PerchEvents_DisplayCalendar {
    ...
    */
    public function __construct($api="", $yr=false, $mo=false)
    {
        ...
        $SettingsEvent = new PerchEvents_Settings($this->api);
        $SettingsEvent = $SettingsEvent->find_by_slug("calendar_offset");

          if($SettingsEvent){

               $this->calendar_start_day=$SettingsEvent->calendar_start_day();
        ...
        $this->start_day    = date('D', $this->start_time);
         ...
         if( $this->calendar_start_day=='S'){

          $this->start_offset = date('w', $this->start_time) ;
    ...
    public function display_day_names()
    {
        ...
        for($i=0; $i<7; $i++) {
          ...
          //   $dayname = date($this->day_name_format, strtotime($names[$i]));
           if( $this->calendar_start_day=='S'){
             $dayname = date($this->day_name_format, strtotime("Sunday +$i days"));
             }else{
              $dayname = date($this->day_name_format, strtotime($names[$i]));
    ...
    public function display_day_cell($day, $dow)
    {
     ...
     $thisdatetime = date("Y-m-d h:i",strtotime($Event->eventDateTime()));
        ...
        $event_days[]= $eventDate;
          ...
          // echo $thisdatetime;
               $Event->squirrel('eventDateTime', $thisdatetime);
